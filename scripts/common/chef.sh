...
#   'provisionerless' -- build a image without Chef
#   'x.y.z'           -- build a image with version x.y.z of Chef
#   'latest'          -- build a image with the latest version of Chef
#   'prerelease'      -- build a image with a prerelease version of Chef
...
if [ x$CHEF_VERSION != x'provisionerless' ]; then
  ...
  echo "Building a image without Chef"
fi
